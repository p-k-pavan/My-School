...
export default function TeacherSchedule({ academicYear = "2026-2027" }: TeacherScheduleProps) {
  const { data, isLoading } = useGetTimetableByTeacherQuery({ academicYear });
  const timetable = data?.timetable;

  const daysOfWeek = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const todayName = daysOfWeek[new Date().getDay()];

  const todayEntries = Array.isArray(timetable)
    ? timetable.filter((t) => t.day.toLowerCase() === todayName.toLowerCase())
    : [];

  const todayPeriods: any[] = [];
