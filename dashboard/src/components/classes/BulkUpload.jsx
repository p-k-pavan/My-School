...
import { useBulkUploadClassMutation } from "@/redux/api/class";

export default function BulkUploadDialog({ open, onClose }) {
    const [file, setFile] = useState(null);
    const [bulkUploadClass, { isLoading }] = useBulkUploadClassMutation();
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await bulkUploadClass(file).unwrap();
            toast.success(response.message || "Classes uploaded successfully");
            setFile(null);
            onClose();
        } catch (error) {
        ...
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Import multiple classes at once using a CSV or Excel sheet.
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <div className="flex gap-2">
                            ...
                            <div className="space-y-1">
                                ...
                                <ul className="list-disc list-inside text-xs text-muted-foreground space-y-1 mt-1 font-mono bg-background/50 p-2 rounded-md">
                                    <li>className (e.g. 1, 2, 10 - must be numeric)</li>
                                    <li>section (e.g. A, B, C)</li>
                                    <li>classTeacher (optional - 24-character Teacher ID)</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <DialogFooter className="flex items-center justify-end gap-2">
                    ...
                    >
                        {isLoading ? "Uploading..." : "Upload Classes"}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
